...
class PoseExtractor:
    ...
    def compute_camera_pose(self, metadata):
        ...
        Correct Rotation: R_yaw(Z) * R_pitch_corrected(Y) * R_roll(X)
        where pitch_corrected = DJI_pitch + 90
        """
        yaw = metadata['GimbalYawDegree']
        pitch = metadata['GimbalPitchDegree'] + 90.0
        roll = metadata['GimbalRollDegree']
        
        R_y = R.from_euler('z', yaw, degrees=True)
        R_p = R.from_euler('y', pitch, degrees=True)
        R_r = R.from_euler('x', roll, degrees=True)
